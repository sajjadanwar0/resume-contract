#!/usr/bin/env bash
# 146_exhaustive_conformance.sh -- exhaustive bounded conformance for remit-core
# ==============================================================================
# Upgrades the package's model-conformance evidence from seeded-randomized to
# EXHAUSTIVE at the reference bounds: full breadth-first enumeration of the
# implementation-level transition system under the R0 configuration (N=3,
# IP=2, values {va,vb}, <=2 forks, <=1 crash, <=1 extra resume, <=1 invalid
# attempt), deduplicated on a canonical state projection, all six invariants
# re-checked at EVERY reachable state -- the implementation-level analogue of
# TLC's exhaustive R0 run.
#
# Verified results (container 2026-07-18; deterministic, replicate on host):
#   default bounds : 6,110 distinct states, 13,035 transitions, depth 10, PASS
#   scaled bounds  : REMIT_EXH_FORKS=3 REMIT_EXH_CRASHES=2 REMIT_EXH_EXTRAS=2
#                    REMIT_EXH_INVALIDS=2 -> 414,675 states, 1,069,437
#                    transitions, depth 14, PASS (~36 s release)
#
# Applies two changes to an existing remit-contract checkout (idempotent):
#   1. derive(Clone) on Plane (needed for BFS branching)
#   2. crates/remit-core/tests/exhaustive_conformance.rs (new test; cargo
#      picks it up automatically -- Rust suite becomes 17 tests)
#
# Usage: bash 146_exhaustive_conformance.sh [PACKAGE_DIR]   # default: remit-contract
set -euo pipefail
PKG="${1:-remit-contract}"
[ -f "$PKG/crates/remit-core/src/lib.rs" ] || { echo "ERROR: $PKG is not a remit-contract checkout"; exit 2; }
cd "$PKG"

python3 - << 'PYPATCH'
s = open('crates/remit-core/src/lib.rs').read()
old = "#[derive(Debug, Default, Serialize, Deserialize)]\npub struct Plane {"
new = "#[derive(Clone, Debug, Default, Serialize, Deserialize)]\npub struct Plane {"
if new in s:
    print("Plane already Clone -- skipping")
elif old in s:
    open('crates/remit-core/src/lib.rs','w').write(s.replace(old, new, 1))
    print("added Clone to Plane")
else:
    raise SystemExit("ERROR: Plane derive line not found in expected form")
PYPATCH

cat > crates/remit-core/tests/exhaustive_conformance.rs << 'EOF_EXH_CONF'
