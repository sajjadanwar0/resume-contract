...
def test_fi_fork_view_matches_probe_134_cells():
    assert fork_view(True, False, True) == "strip"
    assert fork_view(False, True, True) == "strip"
    assert fork_view(False, False, True) == "keep"
    assert fork_view(True, False, False) == "keep"
