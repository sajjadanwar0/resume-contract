#!/usr/bin/env python3
"""
114_p2_conformance_llamaindex.py
Resume-contract conformance probe: LlamaIndex Workflows (pkg `llama-index-workflows`).

Properties:
  PC  prefix consistency  - resuming a serialized mid-run context must not
                            re-execute completed work
  EO  effect exactly-once - side-effect counters must be 1 across snapshot+resume
  FD  fork determinism    - same snapshot resumed with different human responses
                            must yield the two respective outcomes
  CO  consume-once        - two HumanResponseEvents into one live run: post-effect
                            fires once or twice?
  CV  checkpoint validity - non-serializable ctx.store entry: error or silent
                            corruption at snapshot/restore?

Patterns probed:
  A: two-step HITL (step -> InputRequiredEvent, step(HumanResponseEvent) -> Stop)
  B: single-step ctx.wait_for_event(...) durable-HITL pattern
"""
import asyncio
import json
import traceback
...
def make_wf_B(eff):
    class WB(Workflow):
        ...
        async def only(self, ctx: Context, ev: StartEvent) -> StopEvent:
            eff["prefix"] += 1  # side effect BEFORE the durable wait
...
async def c1_nonserializable_store():
    ...
    async for ev in handler.stream_events():
        if isinstance(ev, InputRequiredEvent):
            await handler.ctx.store.set("bad", lambda x: x)  # not serializable
            try:
